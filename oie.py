...
# Le nombre de lignes pour les cases 1 à 63 reste le même :
NB_ROWS = ((NB_CASES - 1) // NB_COLS) + 1
# La hauteur du canvas prend en compte la hauteur maximale entre la première ligne (avec la case départ)
...
SPECIAL_CASES = {
    # ponts
    6: ("pont", 12),
    # cases oies (on considère ici 9, 18, 27, 36, 45, 54)
    9: ("oie", None),
    18: ("oie", None),
    27: ("oie", None),
    36: ("oie", None),
    ...
    19: ("hotel", None),
    # puits
    31: ("puits", 20),  # dans notre version, retourner en 20
    # labyrinthe
    42: ("labyrinthe", 30),  # reculer de 12 cases
    # prison
    52: ("prison", None),
    # tête de mort
    58: ("tete de mort", 0),
...
class GooseGame(tk.Tk):
    def __init__(self):
        ...
        self.current_player = 0  # Index du joueur en cours
        # Pour suivre le numéro de tour individuel (pour la règle des dés spéciaux)
        self.turn_number = {}
    ...
    def setup_start_screen(self):
        ...
        # Pour cet exemple, on propose 2 joueurs
        self.name_vars = []
        for i in range(2):
            var = tk.StringVar()
    ...
    def setup_game_screen(self):
        ...
        # Zone d'information et bouton lancer les dés
    ...
    def draw_board(self):
        ...
        # On la place en position (row=0, col=0)
        x1 = 0
        y1 = 0
        x2 = CELL_SIZE
        y2 = CELL_SIZE
        ...
        # Pour num >= 1, on décale la colonne de 1 vers la droite.
        for num in range(1, NB_CASES+1):
            # Calculer la ligne et la colonne en tenant compte du décalage
            # num=1 -> row=0, num=9 -> row=0, num=10 -> row=1, etc.
            row = (num - 1) // NB_COLS
    ...
    def draw_players(self):
        # Pour chaque joueur, dessiner un cercle (jeton) dans la case correspondante.
        # Si plusieurs joueurs se trouvent sur la même case, on décale leur affichage.
        positions = {}
        for player in self.players:
            pos = player["pos"]
            # Calcul de la position sur le canvas :
            if pos == 0:
                # La case départ est à row=0, col=0
                row, col = 0, 0
            else:
                row = (pos - 1) // NB_COLS
                # Décalage de +1 pour la colonne
                col = ((pos - 1) % NB_COLS) + 1
            # Gestion du décalage si plusieurs joueurs sur la même case
            key = (row, col)
    ...
    def roll_dice(self):
        ...
        # Vérifier si le joueur doit passer son tour (ex : hôtel)
        if p["skip"] > 0:
            messagebox.showinfo(
        ...
        # Si le joueur est en prison, il ne peut jouer qu'en faisant un double
        if p["prison"]:
        ...
        # Lancer des dés normalement
        d1, d2 = random.randint(1, 6), random.randint(1, 6)
        total = d1 + d2
        messagebox.showinfo(
            "Dés", f"{p['name']} lance les dés : {d1} et {d2} (total = {total})")

        # Règle spéciale du premier tour pour les cases "Dés"
        if p["first_turn"]:
            if (d1, d2) in [(6, 3), (3, 6)]:
                messagebox.showinfo(
    ...
    def move_player(self, p, steps, dice=(0, 0)):
        ...
        # Si dépassement de la case finale, reculer du surplus
        if new_pos > NB_CASES:
        ...
        p["pos"] = new_pos
        p["first_turn"] = False  # Dès qu'il a joué, ce n'est plus son premier tour

        # Vérifier si la case est spéciale
        if new_pos in SPECIAL_CASES:
            ...
            if typ == "oie":
                ...
                # Le test de victoire se fait après.
                pass

        # Vérifier la collision avec un autre joueur (échange de places)
        self.check_collision(p, old_pos)

    def check_collision(self, current, old_pos):
        # Si un autre joueur se trouve sur la même case que current, on échange leurs positions
